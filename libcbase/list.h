...
extern int32_t __list_add_valid(struct list_head *newn,
			      struct list_head *prev,
			      struct list_head *next);
extern int32_t __list_del_entry_valid(struct list_head *entry);
#else
static inline int32_t __list_add_valid(struct list_head *newn,
				struct list_head *prev,
				struct list_head *next)
{
	return 1;
}
static inline int32_t __list_del_entry_valid(struct list_head *entry)
{
	return 1;
}
#endif
...
static inline void __list_add(struct list_head *newn,
			      ...
			      struct list_head *next)
{
	//if (!__list_add_valid(newn, prev, next))
	//	return;

	next->prev = newn;
	newn->next = next;
	newn->prev = prev;
	WRITE_ONCE(prev->next, newn);
}
 ...
 */
static inline int32_t hlist_fake(struct hlist_node *h)
{
